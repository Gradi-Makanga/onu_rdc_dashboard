...
ODD_GROUPS <- list(
  odd1  = c("IND001","IND002","IND003","IND004","IND114","IND135","IND136","IND137","IND138","IND139","IND140","IND163","IND164"),
  odd2  = c("IND006","IND007","IND008","IND009","IND010","IND011","IND012"),
  odd3  = c("IND013","IND014","IND015","IND016","IND017","IND018","IND019","IND020","IND021",
  ...
  odd6  = c("IND005"), 
  odd7  = c("IND053","IND054","IND055","IND056","IND057","IND058","IND176","IND177"),
  odd8  = c("IND105","IND060","IND061","IND062","IND063","IND064","IND065","IND066","IND067","IND068","IND069","IND173","IND174","IND175"),
  odd9  = c("IND070","IND072","IND073","IND170","IND171","IND172"),
  odd10 = c("IND074","IND178"),
  odd11 = c("IND075","IND076","IND077","IND130","IND179","IND180"), 
  odd12 = c("IND181"),
  odd13 = character(0),
  odd14 = c("IND182","IND183"),  
  odd15 = c("IND165","IND166","IND167"),
  odd16 = c("IND078","IND079","IND080","IND081","IND082","IND083","IND084","IND129","IND127","IND128"),
  odd17 = c("IND088","IND089","IND090","IND091","IND092","IND144","IND145","IND146","IND147","IND148","IND149","IND150",
"IND151","IND152","IND153","IND154","IND155","IND156","IND157","IND158","IND159","IND160","IND161","IND162","IND168","IND169")
)   
...
SOCIO_CODES <- c(
  ...
  "IND113","IND115","IND116","IND117","IND118","IND119","IND120","IND121","IND122",
  "IND123","IND124","IND125","IND126",
  "IND085","IND086","IND087","IND141","IND142","IND143","IND144","IND053",
...
server <- function(input, output, session){
  ...
  # ===== Cache shapefile (read/transform/valid ONCE) =====
shp_cached <- reactiveVal(NULL)

get_shp_adm2 <- function(){
  g0 <- shp_cached()
  if (!is.null(g0) && inherits(g0, "sf")) return(g0)
  ...
  if (is.null(g) || !inherits(g, "sf")) return(NULL)

  g <- tryCatch(sf::st_transform(g, 4326), error = function(e) g)
  g <- sf::st_make_valid(g)

  # precompute join_key + label_name ONCE
  ...
  # centroid points ONCE (gain fort)
  cent <- sf::st_point_on_surface(g)
  cent_xy <- sf::st_coordinates(cent)
  g$cent_lng <- cent_xy[,1]
  g$cent_lat <- cent_xy[,2]
...
output$last_update_txt <- renderText({
      ...
      if (nrow(dc)) {
        ...
        return(
          ggplot(dat_all, aes(
              ...
              text = paste0("Année: ", period, "<br>Valeur: ", round(value, 2))
            )) +
  ...
  text = element_text(
    family = "",        # police par défaut ggplot (comme les barres)
    face = "plain",
    colour = "#000000"
  ),
  ...
  # --- Interaction (tooltip Année/Valeur) UNIQUEMENT pour les graphiques de tendance (lignes) ---
  is_trend_plot <- reactive({
    dat <- data_rx()
    if (!is.data.frame(dat) || !nrow(dat)) return(FALSE)
    has_sub <- any(!is.na(dat$ref_area)  & dat$ref_area  != "COD")
    has_obs <- any(!is.na(dat$obs_status) & dat$obs_status != "RDC")
    !(has_sub || has_obs)  # TRUE => série temporelle (ligne)
  })
...
output$plotly <- renderPlotly({
  ...
  ggplotly(p, tooltip = "text") %>%
    layout(
      hovermode = "closest",
      # tooltip rectangle blanc (comme tu veux)
      hoverlabel = list(
        bgcolor = "rgba(255,255,255,0.95)",
        bordercolor = "rgba(0,0,0,0.6)",
        font = list(color = "black", size = 12)
      ),
      # ✅ légende en haut (horizontale)
      legend = list(
        orientation = "h",
        x = 0, xanchor = "left",
        y = 1.08, yanchor = "bottom"
      ),
      # un peu de marge en haut pour éviter le chevauchement
      margin = list(t = 60)
    ) %>%
  ...
  if (is.null(g)) {
    return(m |> addControl("Shapefile introuvable : placer gadm41_COD_2.* dans ui/www",
                           position = "bottomleft"))
  }

  # Bordures UNE fois (base layer)
  m |>
    addPolygons(data = g, fillOpacity = 0, color = "#000000", weight = 1, group = "adm2_base")
})
...
observe({
  ...
  # si pas de data -> nettoyer les couches thématiques
  ...
  if (!length(yrs_all)) {
    leafletProxy("map") |>
      ...
      clearGroup("labels") |>
      clearControls() |>
      addControl("Aucune zone détectée pour cet indicateur.", position = "bottomleft")
    return()
  }
  ...
  dat2 <- dat |>
    ...
    dplyr::transmute(join_key = ref_area, value, modality, year = sel_year)
  ...
  if (!nrow(dat2)) {
    leafletProxy("map") |>
      ...
      clearGroup("labels") |>
      clearControls() |>
      addControl("Aucune zone détectée pour cet indicicateur.", position = "bottomleft")
    return()
  }
  ...
  leafletProxy("map") |>
    ...
    addLabelOnlyMarkers(
      lng = gj_on$cent_lng, lat = gj_on$cent_lat, label = gj_on$label_name,
