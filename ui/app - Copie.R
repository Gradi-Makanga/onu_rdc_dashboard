...
# On lit le fichier export_latest.csv dans ui/data_final
# ================== CHARGEMENT DES DONNÉES LOCALES ==================
# On lit le fichier export_latest.csv dans ui/data_final
DATA_ALL <- tryCatch(
  readr::read_csv(
    "data_final/export_latest.csv",
    locale = readr::locale(encoding = "Latin1"),  # <--- AJOUT IMPORTANT
    show_col_types = FALSE
  ),
  error = function(e) {
    message("Impossible de charger data_final/export_latest.csv : ", e$message)
    NULL
  }
)


# On force certains noms de colonnes attendus (au cas où)
if (!is.null(DATA_ALL)) {
  nm <- names(DATA_ALL)
  nm <- tolower(nm)
  names(DATA_ALL) <- nm
  # On suppose qu'il y a au moins :
  # indicator_code, indicator_name, period, value, source, obs_status, ref_area
}
...
fix_utf8 <- function(x){
  ...
  # *** LIGNE CLÉ : corriger Ã + espace insécable -> "à"
  x <- gsub("Ã\u00A0", "à", x, fixed = TRUE)

  # Si tu as déjà le bloc decode_angle_hex_once, garde-le :
  if (any(grepl("<[0-9a-fA-F]{2}>", x, perl = TRUE))) {
    x <- vapply(x, decode_angle_hex_once, "", USE.NAMES = FALSE)
  }

  # Tes autres remplacements existants (é, è, ç, etc.) peuvent rester :
  repl <- c(
    "Ã©" = "é",
    "Ã¨" = "è",
    "Ãª" = "ê",
  ...
  for (i in seq_along(repl)) {
    mauvais <- names(repl)[i]
    bon     <- repl[i]
    x <- gsub(mauvais, bon, x, fixed = TRUE)
  }
...
ODD_GROUPS <- list(
  ...
  odd17 = c("IND088","IND089","IND090","IND091","IND092")
)
...
SOCIO_CODES <- c(
  ...
  "IND084","IND085","IND086","IND087"
)

# ================== FONCTIONS BASÉES SUR DATA_ALL (pas d'API) ==================

# Liste des indicateurs
fetch_indicators <- function(query = "") {
  if (is.null(DATA_ALL) ||
      !all(c("indicator_code","indicator_name") %in% names(DATA_ALL))) {
  ...
  if (nzchar(query)) {
    ...
    df <- df %>%
      filter(
        grepl(q, tolower(indicator_name)) |
          grepl(q, tolower(indicator_code))
      )
  }

  df |> arrange(indicator_name)
}

# Filtrage des valeurs pour l'indicateur principal
fetch_values_local <- function(ind_name, src_vec, years, df_cat) {
  if (is.null(DATA_ALL)) return(tibble())
  if (!nz_chr1(ind_name)) return(tibble())

  code <- df_cat$indicator_code[match(ind_name, df_cat$indicator_name)]
  if (!nz_chr1(code)) return(tibble())

  df <- DATA_ALL

  needed <- c("indicator_code","indicator_name","period","value",
              "source","obs_status","ref_area")
  ...
  if (length(src_vec)) {
    df <- df |> filter(source %in% src_vec)
  }

  df |> arrange(ref_area, period)
}

# Filtrage pour l'indicateur de comparaison
fetch_values_compare_local <- function(comp_name, src_vec, years, df_cat) {
  if (is.null(DATA_ALL)) return(tibble())
  if (!nz_chr1(comp_name)) return(tibble())
  if (!nrow(df_cat)) return(tibble())

  code <- df_cat$indicator_code[match(comp_name, df_cat$indicator_name)]
  if (!nz_chr1(code)) return(tibble())
...
server <- function(input, output, session){
  ...
  # Remplissage initial de la liste d'indicateurs
  observe({
    cf <- code_filter()
    df <- fetch_indicators("")
  ...
  # Quand on change d'indicateur principal => mise à jour des sources disponibles
  observeEvent(input$ind_name, ignoreInit=TRUE, {
    df <- ind_cat()
    if (nrow(df)==0 || !nz_chr1(input$ind_name)) return(NULL)
  ...
  # On ne modifie PLUS la liste des indicateurs quand la source change
  observeEvent(input$source, ignoreInit = TRUE, {
    invisible(NULL)
  })

output$subtitle <- renderText({
    nm <- fix_utf8(input$ind_name)         # <-- correction ici
  ...
  # Données principales
  data_rx <- eventReactive(input$go, {
    df_cat <- ind_cat()
    fetch_values_local(
      ind_name = input$ind_name,
      src_vec  = input$source,
      years    = input$years,
      df_cat   = df_cat
    )
  }, ignoreInit=TRUE)
  ...
  compare_data_rx <- eventReactive(input$go, {
    ...
    df_cat <- ind_cat()
    fetch_values_compare_local(
      comp_name = input$compare_ind,
      src_vec   = input$source,
      years     = input$years,
      df_cat    = df_cat
    )
  ...
  # Téléchargement de la carte (PNG)
