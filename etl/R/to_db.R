...
# ✅ (NOUVEAU) Option pour **insérer aussi les lignes à clés manquantes** dans la table principale
#    en remplissant des sentinelles stables :
#       indicator_code='UNKNOWN', ref_area='UNK', period=-9999, source='unknown',
#       obs_status=coalesce(obs_status,'unknown').
#    -> Ces sentinelles garantissent l'insertion tout en évitant les NULL dans la clé.
#    -> Les conflits éventuels sont ignorés (DO NOTHING) pour ne pas casser la transaction.
#
# Usage direct :
#   source("R/to_db.R"); to_db("data_final", replace_mode = TRUE, include_missing_in_main = TRUE)
# ------------------------------------------------------------------------------
...
# ------------------- remplissage sentinelles pour clés manquantes -----------
fill_missing_keys <- function(df_bad) {
  # Remplace les clés NULL/vides par des sentinelles stables
  df_bad <- df_bad |>
    mutate(
      indicator_code = ifelse(is.na(indicator_code) | !nzchar(indicator_code), "UNKNOWN", indicator_code),
      ref_area       = ifelse(is.na(ref_area) | !nzchar(ref_area),           "UNK",     ref_area),
      period         = ifelse(is.na(period),                                  -9999L,    period),
      source         = ifelse(is.na(source) | !nzchar(source),                "unknown", source),
      obs_status     = ifelse(is.na(obs_status) | !nzchar(obs_status),        "unknown", obs_status)
    )
  df_bad
}
...
# - include_missing_in_main = TRUE => insère aussi les lignes à clés manquantes (avec sentinelles)
# ---------------------------------------------------------------------------
...
to_db <- function(x = "data_final",
  ...
  tryCatch({
    ...
    # (NOUVEAU) Insérer aussi les lignes avec clés manquantes dans MAIN (avec sentinelles)
    if (include_missing_in_main) {
      step <- "insert missing -> main"
      df_bad <- df_all[!is.na(df_all$reason), , drop = FALSE]
      if (nrow(df_bad)) {
        df_bad_filled <- fill_missing_keys(df_bad) |> select(-reason)

        tmp_bad <- paste0("tmp_bad_", as.integer(as.numeric(Sys.time())))
        DBI::dbExecute(con, sprintf(
          "CREATE TEMP TABLE \"%s\" (
             indicator_code TEXT, ref_area TEXT, period INTEGER, value DOUBLE PRECISION,
             obs_status TEXT, source TEXT, indicator_name TEXT
           ) ON COMMIT DROP;", tmp_bad))
        DBI::dbWriteTable(con, tmp_bad, utf8_sanitize_df(df_bad_filled), append = TRUE, row.names = FALSE)

        # On évite toute erreur de conflit: DO NOTHING
        DBI::dbExecute(con, sprintf(
          "INSERT INTO \"%1$s\".\"%2$s\"(indicator_code, ref_area, period, value, obs_status, source, indicator_name)
           SELECT indicator_code, ref_area, period, value, obs_status, source, indicator_name FROM \"%3$s\"
           ON CONFLICT (indicator_code, ref_area, period, source, value, obs_status) DO NOTHING;",
          schema, table_main, tmp_bad))
      }
    }

    DBI::dbCommit(con)
    message(sprintf('[to_db] OK | total=%d | key_ok=%d | key_bad=%d | mode=%s | missing->main=%s',
                    nrow(df_all), sum(key_ok), sum(!key_ok), ifelse(replace_mode, "REPLACE", "UPSERT"),
                    include_missing_in_main))
    invisible(TRUE)
  }, error = function(e) {
